...
def greedyBFS(G, source, dest, heuristics, pos):
    ...
    for var in final_path:
        if prev != -1:
            curr = var
            nx.draw_networkx_edges(G, pos, edgelist = [(prev, curr)], width = 2.5, alpha = 0.5, edge_color = 'black')
            prev = curr
        else:
            prev = var
    return
...
def DrawPath(G, source, dest):
    ...
    nx.draw(G, pos, with_label = True, node_color = values, edge_color = 'b', width = 1, alpha = 0.7)
